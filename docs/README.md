...
| `docs/`       | A Python directory that contains code for your Dagster repository                 |
| `docs_tests/` | A Python directory that contains tests for `docs`                      |
...
pyenv virtualenv $PYTHON_VERSION docs
pyenv activate docs
```
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name docs python=PYTHON_VERSION
conda activate docs
```
...
Tests can be found in `docs_tests` and are run with the following command:

```bash
pytest docs_tests
```

As you create Dagster ops and graphs, add tests in `docs_tests/` to check that your
